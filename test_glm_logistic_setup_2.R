...
# reasonable distance threshold for land use proximity? This should be in feet
quantile(tree_vars_full_nona$pluto_dist, seq(0, 1, 0.01))
# picking 20, this is approx 95% of the remaining trees
...
tree_vars_full_nona_format$tree_dbh_bin <- cut(tree_vars_full_nona_format$tree_dbh, c(0, 3, 6, 9, 12, 18, 24, 500))
...
sp <- "Acer rubrum" # "Prunus" #"Quercus palustris" #"Acer platanoides"
sp_sub_format <- tree_vars_full_nona_format %>% filter(species == sp)
mort_model <- glm( canopy_endstate ~ imp_10_2021 + tree_dbh_bin + is_DM_bool + steward_level + in_sandy_zone_bool + LandUse_char_collapsed + RPL_THEME1_bin + RPL_THEME2_bin + RPL_THEME3_bin + RPL_THEME4_bin +
                   tree_dbh_bin*imp_10_2021 + tree_dbh_bin*steward_level + tree_dbh_bin*is_DM_bool,
                   family = binomial(link = "cloglog"),
                   data = sp_sub_format)
# Note, with interaction, this warning appears:
# Warning message:
